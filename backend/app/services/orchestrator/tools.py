...
# -----------------------------
def parse_time_expression(time_expression: str, duration_minutes: int) -> Tuple[datetime, datetime]:
    """Resolve a human-readable time expression into start_dt and end_dt (UTC)."""
    try:
        cal = parsedatetime.Calendar()
        now = datetime.now(timezone.utc)
        parsed_time, status = cal.parseDT(time_expression, sourceTime=now)
        if not status:
            raise
...
async def list_events(
    ...
    start_dt, end_dt = parse_time_expression(time_expression, duration_minutes or 60)
    command = EventListCommand(start_dt=start_dt, end_dt=end_dt, limit=limit or 10)
