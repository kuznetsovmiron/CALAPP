...
class UserService:
    ...
    async def list(cls) -> list[UserDTO]:
        ...
        try:
            ...
            logger.exception(f"Failed to list users: {e}", exc_info=e)
    ...
    async def retrieve(cls, user_id: UUID) -> UserDTO: 
        ...
        try:
            ...
            logger.exception(f"Failed to retrieve user with user_id={user_id}: {e}", exc_info=e)
    ...
    async def retrieve_by_email(cls, email: EmailStr) -> UserDTO:
        ...
        try:
            ...
            logger.exception(f"Failed to retrieve user by email={email}: {e}", exc_info=e)
            raise InternalError("Failed to retrieve user")
    ...
    async def create(cls, data: UserCreateDTO) -> UserDTO:
        ...
        try:             
            ...
            logger.exception(f"Failed to create user with data={data}: {e}", exc_info=e)
    ...
    async def update(cls, user_id: UUID, data: UserUpdateDTO) -> UserDTO:
        ...
        try:   
            ...
            logger.exception(f"Failed to update user user_id={user_id}, data={data}: {e}", exc_info=e)
    ...
    async def update_password(cls, user_id: UUID, data: UserUpdatePasswordDTO) -> UserDTO:
        ...
        try:
            ...
            logger.exception(f"Failed to update user password for user_id={user_id}, data={data}: {e}", exc_info=e)
